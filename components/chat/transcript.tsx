  ...
  return (
    <AnimatePresence mode="wait">
      <div
        ...
        className={cn(
          "rounded-xl p-3 md:p-6 flex flex-col justify-between z-20 backdrop-blur-md border transition-colors duration-300",
          videoOn
      ...
      >
        <div key={displayedText} className="overflow-y-auto max-h-48 md:max-h-64">
          <p
            className={cn(
          ...
          >
            {loading ? "Generating Response..." : displayedText || "Your text will display here..."}

            {showCursor && <span className="inline-block w-[4px] h-4 bg-gray-800 dark:bg-white ml-1" />}
          </p>
        </div>
      </div>
    </AnimatePresence>
